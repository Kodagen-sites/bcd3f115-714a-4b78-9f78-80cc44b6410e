...
export default function HeaderSplitEdges() {
  ...
  // Home opens on the dark scrub hero (transparent bar reads well); every other
  // route has a light page, so pin the dark bar to keep the cream text legible.
    ...
    <>
      <motion.header
        ...
        className={`fixed inset-x-0 top-0 z-40 pointer-events-none transition-all duration-500 ${
          solidBar ? "bg-[#0E1C33]/75 backdrop-blur-md" : "bg-transparent"
        }`}
      >
        <div className="flex items-start justify-between p-6 md:p-8 pointer-events-auto">
          ...
          <Link
            href="/"
            className="group flex flex-col gap-1 text-white hover:text-[#E8DFCF] transition-colors"
          >
          ...
          {!isMobile ? (
            <div className="flex items-start gap-10">
                  ...
                  return (
                    <Link
                      ...
                      className={`font-mono text-[11px] uppercase tracking-[0.25em] transition-colors ${
                        active ? "text-[#E8DFCF]" : "text-white/80 hover:text-white"
                      }`}
                    >
                      {link.label}
                    </Link>
              ...
              <Link
                href="/#contact"
                className="font-display text-xs uppercase tracking-[0.2em] text-white border-b border-white/30 hover:border-[#E8DFCF] hover:text-[#E8DFCF] transition-colors pb-1"
              >
                Start →
              </Link>
            </div>
...
function MobileFullscreen({ onClose }: { onClose: () => void }) {
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 bg-[#0E1C33] md:hidden"
    >
        ...
        {NAV_LINKS.map((link, i) => (
          ...
          >
            <Link
              href={link.href}
              onClick={onClose}
              className="font-display text-4xl text-white hover:text-[#E8DFCF] transition-colors"
            >
              {link.label}
            </Link>
          </motion.li>
